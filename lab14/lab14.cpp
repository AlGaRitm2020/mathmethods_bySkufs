...
#define N_cities 100  //количество городов
...
// lab14 алгоритм Метрополиса
void metropolis(int cities[], double r[][N_cities], int x[], int y[])
{
    int c1, c2;
    double path_new, path_old;
    double f; // вероятность f=exp(-(path_new-path_old)/T)
    double R; //[0..1)

    path_old = calc_path(cities, r);
	file_write(x,y,cities,"first.txt");

	ofstream all("all.txt");

    double path_aver; // средний путь для подбора диапазона температуры
	int ii = 0;
	ofstream gnuplot_file("gnup_multifora.gp");
    for (double T = 10000; T > 1; T /= 1.05) // оптимальный шаг по температуре T/=1.05 для плавного понижения температуры
    {
        path_aver = 0;
        for (unsigned int step = 1; step <= MC_steps; step++)
        {
            ...
            path_new = calc_path(cities, r); // вычисляем новую длину пути

            // если путь уменьшился, то принимаем обмен и переходим к новому шагу
            // если путь увеличился, то включаем вероятность:
            if (path_new - path_old > 0)
            {
                f = exp(-(path_new - path_old) / T);
                R = (double)rand() / RAND_MAX; // 0..1

                if (R == 0 || R > f) // при R==0 нарушается работа алгоритма, т.к. f>0 всегда
                {
                    change(cities, c1, c2); // возвращаем города на свои места
                    path_new = path_old;    // для избежания пересчета path_old ниже
                }
				else{
					/*
					for (int i = 0; i < n; i++)
            ...
            path_old = path_new; // сохраняем путь для следующего шага

            path_aver += (path_old - path_aver) / step; // считаем средний путь для данной температуры

			ii++;
        } // MC


		int min_path = path_old;
			cout << "min_path = " << min_path << endl;
			string filename = "progress/"+to_string(ii)+".txt";
			string filename_png = "progress/"+to_string(ii)+".png";
            ...
            gnuplot_file << "set title \"Temp: " << T << "  step: " << ii << "\" font \"Verdana,20\" " << endl;
        ...
        cout << T << "\t" << path_aver << endl; // смотрим как изменяется средний путь, чтобы подобрать температуру
    } // T
